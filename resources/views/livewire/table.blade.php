...
<div class="mt-8 flow-root space-y-3">
                ...
                <table class="min-w-full divide-y divide-gray-300">
                    ...
                    <tr>
                        @foreach($this->columns() as $column)
                            <th scope="col">
                                <div class="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-50 sm:pl-6">
                                    {{ $column->label }}
                                </div>
                            </th>
                        @endforeach
    ...
    {{--    <div class="flex justify-between items-center" x-data="{ current_page: {{ $this->paginate() }} }">--}}
    {{--        <span class="mr-4">Page {{ $this->paginate() }} of {{ $this->paginate() }}</span>--}}
    ...
    {{--                    :class=" { 'cursor-not-allowed': {{ $this->paginate() }} === 1 }"--}}
    {{--                    x-on:click="$wire.paginate({{ $this->paginate() - 1 }}); $wire.update()"></button>--}}
    {{--            @foreach(range(1, $this->paginate()) as $page_number)--}}
    ...
    {{--                    :class=" { 'cursor-not-allowed': {{ $this->paginate() }} === {{ $this->paginate() }} }"--}}
